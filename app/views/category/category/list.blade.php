		...
		<div class="mainbox-body" >
			<div>
				<form action="{{$current_url}}" method="GET">
					...
					<div class="summary">
					<?php 
						$start = ($category->getCurrentPage() - 1) * $category->getPerPage() + 1; 
						$end = $start + $category->count() - 1 
					?>
					{{'Displaying ' .  $start . '-' . $end . ' of ' . $category->getTotal() . ' results.'}}
					</div>
					<table class="items">
						<thead>
							<tr>
								...
								<th width="20%">{{SortableTrait::link_to_sorting_action("title", "Title")}}</th>								
								<th width="30%">Thumbnail</th>								
								<th width="10%">{{SortableTrait::link_to_sorting_action("sequence", "Sequence")}}</th>
								<th width="10%">{{SortableTrait::link_to_sorting_action("created_at", "Registered")}}</th>	
					...
					<div class="pager">
						Go to page
						<ul class="yiiPager">
							<?php echo with(new ZurbPresenter($category->appends(Input::except('page'))))->render(); ?>																	
						</ul>
					</div>
